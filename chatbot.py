...
def get_response(key, location="", food="", user_input=""):
    global responses, intendFood, intendLocation, level
    ...
    if level == 1 :
        ...
        return f'{location}의 {food}를 입력하신 것이 맞으신가요? <br> 네/아니오 로 답변해주세요.'

    if level == 2 :
        if key =='네' :
            level = 3
            return "원하시는 식당의 특성을 한 문장으로 말씀해주세요. <br> 예) 데이트하기 좋은 맛집을 알려줘."
        else :
            food = ""
            location = ""
            level = 1
            return "원하시는 식당의 위치와 메뉴를 알려주세요."

    if level == 3 :
        level = 4
        return f'{user_input} 가 원하시는 식당의 특성 맞으신가요? <br> 네/아니오 로 답변해주세요.'

    if level == 4 :
        if key =='네' :
            level = 5
            name, category, restaurantloc = data.searchRestaurant(location, food, user_input)
...
def pick_response(user_input):  # 키워드에 따른 의도 파악 & 의도에 따른 응답 생성
    ...
    response = get_response(key, intendLocation, intendFood, user_input)
    return response
